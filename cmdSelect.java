 ...
 *
 * 11/18/17
 * This command selects tuples from a relation that meet a condition.
 */
import java.util.*;

public class cmdSelect extends base {
	public cmdSelect(Scanner line, Database db, LinkedList<String> errorsList) {
	...
	public void run(Scanner line, Database db, LinkedList<String> errorsList) {
		...
		if (rIndex[0] != -1) {
			...
			Relation tmp = createTmpRelation(r, db, line);
		}
	}

	public Relation createTmpRelation(Relation r, Database db, Scanner line) {
		...
		if (line.hasNext() && nextSymbol(line).equals("WHERE")) {
			qualificationTest(r, tmp, db, line);
		}
	...
	// Run qualification test and weed out tuples that don't match
	public void qualificationTest(Relation r, Relation tmp, Database db, Scanner line) {
		...
		for (int i = 0; i < r.getTuples().size(); i++) {
			...
			boolean pass = runTest(r, t, qualifiers);
			if (pass) {
				tmp.getTuples().add(t);
			}
		}
